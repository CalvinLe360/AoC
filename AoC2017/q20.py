...
p0 = [[int(y) for y in x.split("=<")[1].rstrip(">").split(",")] for x in particles[0].split(", ")]
p0newPos = [p0[0][i] + p0[1][i] + p0[2][i] for i in range(3)]
particleCount = 0

for i in range(1, 200):
    particlePos = [int(x) for x in particles[i].split(", ")[0].lstrip("p=<").rstrip(">").split(",")]
    if sum([1 for x, y in zip(p0newPos, particlePos) if x == y]) != 0:
        print(i)
        break
