...
def truth(variable, comparator, value):
    if comparator == "<":
        return registers[variable] < value
    elif comparator == ">":
        return registers[variable] > value
    elif comparator == "==":
        return registers[variable] == value
        ...
        print(comparator, " not implemented")
...
for i in instructions:
    ...
    if segments[0] not in registers:
        registers[segments[0]] = 0
    if segments[4] not in registers:
        registers[segments[4]] = 0

    truthValue = truth(segments[4], segments[5], int(segments[6]))
    if truthValue:
        if segments[1] == "inc":
            registers[segments[0]] += int(segments[2])
        else:
            registers[segments[0]] -= int(segments[2])
    
    highestAchieved = max(highestAchieved, max(registers.values()))

print(max(registers.values()), highestAchieved)