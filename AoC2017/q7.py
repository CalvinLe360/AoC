...
class Node:
    def __init__(self, name, weight):
        ...
        self.programsWeight = 0

    def findAndReplace(self, nodeName, node):
        if self.name == nodeName:
            ...
            for x in self.connected:
                result = x.findAndReplace(nodeName, node)
                if result is True:
                    return True

    def calibrateWeight(self):
        for x in self.connected:
            self.programsWeight += x.calibrateWeight()

        if len(self.connected) > 0:
            balance = self.programsWeight % (self.connected[0].programsWeight + self.connected[0].weight)
            if balance != 0:
                print(self.weight, [[x.programsWeight + x.weight, x.weight] for x in self.connected])

        return self.programsWeight + self.weight

def solution(p: list[str]):
    nodePool: list[Node] = []

    for x in p:
        info = x.split()
        newNode = Node(info[0], int(info[1][1:-1]))

        if len(info) > 2:
            for i in range(3, len(info)):
                newNode.connected.append(Node(info[i].split(",")[0], 0))

        nodePool.append(newNode)

    while(len(nodePool) > 1):
        chosenNode = nodePool.pop(0)
        applied = False
        for x in nodePool:
            if x.findAndReplace(chosenNode.name, chosenNode) == True:
                applied = True
                break

        if applied == False:
            nodePool.append(chosenNode)

    # print(len(nodePool))
    print("Top of stack - ", nodePool[0].name)
    return nodePool
        

        

solution(programs)[0].calibrateWeight()