...
input.forEach(_command => {
  ...
  for (let x = command.x1; x <= command.x2; x++) {
    for (let y = command.y1; y <= command.y2; y++) {
      ...
      if (command.command === 'turn on') LIGHTS[index] = LIGHTS[index] + 1;
      if (command.command === 'turn off') LIGHTS[index] = Math.max(LIGHTS[index] - 1, 0);
      if (command.command === 'toggle') LIGHTS[index] = LIGHTS[index] + 2;
    }
  }
});

// Calculate total of enabled lights
const result = LIGHTS.reduce((total, light) => light === 0 ? total : total += light, 0);
